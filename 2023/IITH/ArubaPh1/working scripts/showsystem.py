...
# Define the task to run the show system command
def run_show_system(task):
    ...
    try:
        ...
        if vsf_members:
            for member in vsf_members:
                ...
                write_to_csv(task.host.name, task.host.hostname, "Command Output", system_name, software_version, mac_addr, serial_number)
...
def write_to_csv(hostname, ip_address, status, system_name="", software_version="", mac_addr="", serial_number=""):
    csv_file = 'task_status.csv'
    file_exists = os.path.isfile(csv_file)
    with open(csv_file, mode='a', newline='') as file:
        writer = csv.writer(file)
        if not file_exists or os.path.getsize(csv_file) == 0:
            writer.writerow(["Hostname", "IP Address", "Status", "System Name", "Software Version", "MAC Address", "Serial Number"])
        writer.writerow([hostname, ip_address, status, system_name, software_version, mac_addr, serial_number])
...
result = nr.run(task=run_show_system)

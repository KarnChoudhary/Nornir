...
lldp_neighbors_dir = os.path.join(backup_dir, "lldp_neighbors")
...
os.makedirs(lldp_neighbors_dir, exist_ok=True)
...
def get_cdp_neighbors(task: Task) -> Result:
    result = task.run(
        task=netmiko_send_command,
        command_string="show cdp neighbors detail | i Device ID: | IP address:"
    )
    return Result(host=task.host, result=result.result)
...
# Define a task to get LLDP neighbors details
def get_lldp_neighbors(task: Task) -> Result:
    result = task.run(
        task=netmiko_send_command,
        command_string="show lldp neighbors detail | i Device ID: | IP address:"
    )
    return Result(host=task.host, result=result.result)
...
# Define a task to save the LLDP neighbors details to a file
def save_lldp_neighbors(task: Task) -> Result:
    lldp_neighbors = task.host.data["lldp_neighbors"]
    hostname = task.host.name
    filename = os.path.join(lldp_neighbors_dir, f"{hostname}_lldp_neighbors.txt")
    with open(filename, "w") as f:
        f.write(lldp_neighbors)
    return Result(host=task.host, result=f"Saved LLDP neighbors details to {filename}")
...
lldp_neighbors_results = nr.run(task=get_lldp_neighbors)
...
for host, result in lldp_neighbors_results.items():
    nr.inventory.hosts[host].data["lldp_neighbors"] = result.result

nr.run(task=save_running_config)
nr.run(task=save_arp_inspection)
nr.run(task=save_cdp_neighbors)
nr.run(task=save_vlan)
nr.run(task=save_lldp_neighbors)
